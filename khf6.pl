...
% fa_szamlalas([], _, [], [], []) :- 
%     write_ln("rekurzio vege")
%     .

% fa_szamlalas([FaHead|FaTail], Osszegfeltetel, [IlHead|IlTail], [BHead|BTail], [EHead|ETail]) :-
    % (
    %         biztos_teljesul(FaHead, Osszegfeltetel, IlHead) ->         
    %             (
    %                 write_ln('biztos teljesul'),
    %                 BHead = FaHead,
    %                 fa_szamlalas(FaTail, Osszegfeltetel, IlTail, BTail, [EHead|ETail]))
    %     ;   esetleg_teljesul(FaHead, Osszegfeltetel, IlHead) -> 
    %             ( 
    %                 write_ln('esetleg teljesul'),
    %                 EHead = FaHead,
    %                 fa_szamlalas(FaTail, Osszegfeltetel, IlTail, [BHead|BTail], ETail))
    %     ;   (
    %             write_ln('nem teljesul'),
    %             fa_szamlalas(FaTail, Osszegfeltetel, IlTail, [BHead|BTail], [EHead|ETail])
    %         )
    % ).

% szukiti esetlegesen a fakat

esetleg_szukites([], _, _, _, []).
esetleg_szukites([F|Fs], sor(I, Db), [IL0|ILs0], EFs, [IL|ILs]) :-
    (   member(F, EFs) ->  esetlegesbol_biztos(F, sor(I, Db), IL0, IL)
    ;   IL = IL0
    ),
    esetleg_szukites(Fs, sor(I, Db), ILs0, EFs, ILs).
esetleg_szukites([F|Fs], oszl(J, Db), [IL0|ILs0], EFs, [IL|ILs]) :-
    (   member(F, EFs) ->  esetlegesbol_biztos(F, oszl(J, Db), IL0, IL)
    ;   IL = IL0
    ),
    esetleg_szukites(Fs, oszl(J, Db), ILs0, EFs, ILs).
...
biztos_teljesul(_-Fy, oszl(J, _), IL0) :-
(   Fy == J ->       \+ member(e, IL0), \+ member(w, IL0)
;   Fy =:= J - 1 -> IL0 == [e]
;   Fy =:= J + 1 -> IL0 == [w]
).
biztos_teljesul(Fx-_, sor(I, _), IL0) :-
(   Fx == I ->       \+ member(n, IL0), \+ member(s, IL0)
;   Fx =:= I - 1 -> IL0 == [s]
;   Fx =:= I + 1 -> IL0 == [n]
).
